...
const portfolioReadyNews = (news: News) => {
...
const readyNews = (value: unknown) => asArray<News>(value).filter(portfolioReadyNews);
...
function NewsThumb({ news, className }: { news: News; className: string }) {
  ...
  const fitClass = /\bobject-(cover|contain|fill|none|scale-down)\b/.test(className) ? '' : 'object-cover';
  return <img src={image} alt={news.title} className={`${className} ${fitClass}`} />;
}
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const fetchHome = async () => {
      ...
      try {
        ...
        setHomeFeed({
          mainNews: readyNews((homeData as Partial<HomeFeed>).mainNews),
          urgentNews: readyNews((homeData as Partial<HomeFeed>).urgentNews),
          recommendedNews: readyNews((homeData as Partial<HomeFeed>).recommendedNews),
          weeklyPopular: readyNews((homeData as Partial<HomeFeed>).weeklyPopular),
          newsletterPreview: readyNews((homeData as Partial<HomeFeed>).newsletterPreview),
        });
        setLatestNews(readyNews(newsRes.data?.news));
  ...
  useEffect(() => {
    ...
    const fetchByCategory = async () => {
      ...
      try {
        ...
        setLatestNews(readyNews(res.data?.news));
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      if (activeCategoryId !== null) params.categoryId = activeCategoryId;
      const res = await api.get('/news', { params });
      setLatestNews((prev) => [...prev, ...readyNews(res.data?.news)]);
      setPage(nextPage);
          ...
          <section className={`grid gap-4 ${editorPickedNews.length > 0 ? 'lg:grid-cols-[1.4fr_0.8fr]' : ''}`}>
            <Link href={`/news/${featuredNews.id}`} className="group overflow-hidden rounded-2xl border border-gray-200 bg-gray-50 transition hover:border-blue-400 dark:border-gray-800 dark:bg-gray-900">
              <div className="relative h-[22rem] max-h-[58vh] min-h-72">
                <NewsThumb news={featuredNews} className="h-full w-full object-contain sm:object-cover" />
